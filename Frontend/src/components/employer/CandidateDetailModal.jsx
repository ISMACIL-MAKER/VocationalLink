    ...
    <Modal isOpen={!!application} onClose={onClose} title="Candidate Details" maxWidth="max-w-2xl">
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="font-bold text-[#191C1E] text-base">{seeker.username}</h3>
            <p className="text-xs text-[#64748B] mt-0.5">{seeker.email}</p>
            {seeker.phone && <p className="text-xs text-[#64748B]">{seeker.phone}</p>}
        ...
        {skills.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {skills.map((skill) => (
              <span
                key={skill._id}
                className="text-[10px] font-semibold bg-[#F2F4F6] text-[#00236F] px-2.5 py-1 rounded-full flex items-center gap-1"
              >
        ...
        <div className="border-t border-[#F2F4F6] pt-4">
          <h4 className="text-xs font-bold text-[#191C1E] mb-2">Update Pipeline Stage</h4>
          <div className="flex gap-2">
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value)}
              className="flex-1 border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm capitalize"
            >
            ...
            <button
              ...
              className="bg-[#00236F] hover:bg-[#1E3A8A] text-white text-sm font-bold px-4 py-2 rounded-lg"
            >
              Update
            </button>
          </div>
        ...
        {/* FEEDBACK NOTES */}
        <div className="border-t border-[#F2F4F6] pt-4">
          <h4 className="text-xs font-bold text-[#191C1E] mb-2">Feedback Notes</h4>
          {application.feedbackNotes?.length > 0 && (
            <div className="space-y-2 mb-3">
              {application.feedbackNotes.map((note) => (
                <div key={note._id} className="bg-[#F8FAFC] rounded-lg p-3 text-xs text-[#334155]">
                  {note.note}
                </div>
              ))}
            </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-[#F2F4F6] text-[#00236F] text-sm font-bold px-4 py-2 rounded-lg h-fit"
            >
              Add
            </button>
          </div>
        ...
        <div className="border-t border-[#F2F4F6] pt-4">
          <h4 className="text-xs font-bold text-[#191C1E] mb-2">Schedule Interview</h4>
          ...
          <button
            ...
            className="mt-2 bg-[#00236F] hover:bg-[#1E3A8A] text-white text-sm font-bold px-4 py-2 rounded-lg"
          >
            Schedule
          </button>
        </div>
        ...
        {/* MESSAGING */}
        <div className="border-t border-[#F2F4F6] pt-4">
          <h4 className="text-xs font-bold text-[#191C1E] mb-2">Direct Message</h4>
          <div className="bg-[#F8FAFC] rounded-lg p-3 max-h-48 overflow-y-auto space-y-2 mb-3">
            {messages.length === 0 ? (
              <p className="text-xs text-[#94A3B8]">No messages yet. Say hello!</p>
            ) : (
              messages.map((msg) => {
                  ...
                  <div key={msg._id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-[75%] px-3 py-2 rounded-lg text-xs ${
                        isMine
                          ? "bg-[#00236F] text-white rounded-br-none"
                          : "bg-white border border-[#E2E8F0] text-[#191C1E] rounded-bl-none"
                      }`}
                    >
                      {msg.text}
                    </div>
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={sending}
              className="bg-[#00236F] hover:bg-[#1E3A8A] disabled:bg-[#94A3B8] text-white px-4 py-2 rounded-lg"
