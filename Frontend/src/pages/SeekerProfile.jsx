...
export default function SeekerProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage("Seeker profile updated successfully.");
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handleAddSkill = () => {
    ...
    if (!cleaned || skills.includes(cleaned)) return setSkillInput("");
  ...
  const fileToBase64 = (file, cb) => {
    ...
    reader.onloadend = () => cb(reader.result?.toString() || "");
  ...
  return (
    <div className="w-full bg-[#F8FAFC] min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-[#1E3A8A] text-2xl font-bold">Seeker Profile</h1>
        <button
          type="button"
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#E2E8F0] p-6 max-w-3xl">
          <div className="mb-6 flex items-center gap-4">
            ...
            <input
              type="file"
              accept="image/*"
              onChange={(e) => e.target.files?.[0] && fileToBase64(e.target.files[0], setProfileImage)}
              className="text-xs"
            />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input value={username} onChange={(e) => setUsername(e.target.value)} className="border border-[#CBD5E1] rounded-lg px-3 py-2 text-sm" placeholder="Full name" />
            <input value={user?.email || ""} disabled className="border border-[#CBD5E1] rounded-lg px-3 py-2 text-sm bg-[#F8FAFC]" />
          </div>

          <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} className="w-full border border-[#CBD5E1] rounded-lg px-3 py-2 text-sm mt-4" placeholder="Bio" />
          ...
          <div className="mt-4">
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={handleAddSkill} className="px-4 py-2 rounded-lg bg-[#1D4ED8] text-white text-sm font-semibold">Add</button>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              {skills.map((skill) => (
                <span key={skill} className="text-xs bg-[#DBEAFE] text-[#1E40AF] px-3 py-1 rounded-full flex items-center gap-2">
                  {skill}
          ...
          <div className="mt-4">
            <input
              ...
              onChange={(e) => {
                ...
                if (!file) return;
                setCvName(file.name);
                fileToBase64(file, setCvFile);
              }}
              className="text-xs"
            />
            {cvName && <p className="text-xs text-[#475569] mt-2">Current CV: <span className="font-semibold">{cvName}</span></p>}
            {cvFile && <a href={cvFile} download={cvName || "my-cv"} className="inline-block mt-2 text-xs text-[#1D4ED8] font-semibold">Download Uploaded CV</a>}
          </div>
          ...
          <button type="submit" disabled={loading} className="mt-6 bg-[#00236F] hover:bg-[#1E3A8A] disabled:bg-[#94A3B8] text-white px-5 py-2.5 rounded-lg text-sm font-bold">
            {loading ? "Saving..." : "Save Seeker Profile"}
          </button>
        </form>
      )}
    </div>
